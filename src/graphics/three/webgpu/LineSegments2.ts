...
import { Box3, InstancedInterleavedBuffer, InterleavedBufferAttribute, Line3, MathUtils, Matrix4, Mesh, Sphere, Vector3, Vector4, Line2NodeMaterial, Camera } from 'three/webgpu';
...
function getWorldSpaceHalfWidth(camera: Camera, distance: number, resolution: { width: number; height: number }) {
...
function raycastWorldUnits(lineSegments: LineSegments2, intersects: { point: Vector3; pointOnLine: Vector3; distance: any; object: any; face: null; faceIndex: number; uv: null; uv1: null }[]) {
...
function raycastScreenSpace(
  ...
  camera: { projectionMatrix: any; near: number; matrixWorldInverse: Matrix4 },
  intersects: { point: Vector3; pointOnLine: Vector3; distance: any; object: any; face: null; faceIndex: number; uv: null; uv1: null }[]
) {
...
export class LineSegments2 extends Mesh<LineSegmentsGeometry, Line2NodeMaterial> {
  public readonly isLineGeometry = true;
  public readonly isLineSegments2 = true;

  constructor(geometry = new LineSegmentsGeometry(), material = new Line2NodeMaterial({ color: Math.random() * 0xffffff })) {
    super(geometry, material);

    (this as any).type = 'LineSegments2';
  }
  ...
  updatePosition(array: number[]) {
    ...
    buffer.set(points, 0);
    this.geometry.instanceCount = array.length / 6;
